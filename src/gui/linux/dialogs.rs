//! File dialogs, alert helpers, and clipboard operations for Linux GTK GUI.
...
pub fn copy_to_clipboard(text: &str) {
    if let Ok(c_text) = CString::new(text) {
        unsafe {
            ffi::linux_gui_copy_clipboard(c_text.as_ptr());
        }
    }
}
...
pub fn show_alert(title: &str, message: &str) {
    ...
    unsafe {
        ffi::linux_gui_show_alert(c_title.as_ptr(), c_msg.as_ptr());
    }
}
...
pub fn open_dump_file_dialog() {
    unsafe {
        ffi::linux_gui_open_file_dialog();
    }
}
...
pub fn export_dump_dialog(default_filename: &str) {
    ...
    unsafe {
        ffi::linux_gui_save_file_dialog(c_def.as_ptr());
    }
}
