//! Window management, event loop, and render pipeline for Linux GTK GUI.
...
fn update_status_bar(state: &AppState, cpu: &Cpu) {
    ...
    unsafe {
        ffi::linux_gui_set_status(c_p1.as_ptr(), c_p2.as_ptr(), c_p3.as_ptr());
    }
}
...
pub fn run() {
    ...
    let init_ok = unsafe { ffi::linux_gui_init(c_title.as_ptr(), win_w, win_h, min_w, min_h) };

    if !init_ok {
        eprintln!("Failed to initialize Linux GTK GUI.");
        return;
    }

    unsafe {
        ffi::linux_gui_set_callbacks(
